...
from .data_augmenter import CustomImageDataGenerator
...
class ImageFolderDataset(Dataset):
    ...
    def make_dataset(self, directory, class_to_idx, mode):
        ...
        if mode == 'train':
            ...
            augmented_images_labels = [(custom_generator.apply_augmentation(image),label) for (image,label) in zip(images, labels)]
            augmented_images_list = np.array([image_label[0] for image_label in augmented_images_labels])
            augmented_labels_list = np.array([image_label[1] for image_label in augmented_images_labels])
            images = np.concatenate((images, augmented_images_list), axis=0)
            labels = np.concatenate((labels, augmented_labels_list), axis=0)
    ...
    def make_dataset(self, directory, class_to_idx, mode):
        ...
        if mode == 'train':
            ...
            for (image, label) in zip(images, labels):
                aug_img1, aug_img2 = custom_generator.apply_augmentation(image)
                augmented_images_labels.append((aug_img1, label))
                augmented_images_labels.append((aug_img2, label))
