...
class ClassificationNet(Network):
    ...
    def __init__(self, activation=Relu, num_layer=2,
                 input_size=3 * 32 * 32, hidden_sizes=[100], hidden_size = 100,
                 std=1e-3, num_classes=10, reg=0, **kwargs):
        """
        :param activation: choice of activation function. It should implement
            a forward() and a backward() method.
    ...
    def reset_weights(self):
        ...
        for i in range(self.num_layer):
            ...
            self.grads['b' + str(i + 1)] = 0.0
    # def reset_weights(self):
    #     self.params = {'W1':self.std * np.random.randn(self.input_size, self.hidden_sizes[0]),
    #                    'b1': np.zeros(self.hidden_sizes[0])}

    #     for i in range(self.num_layer - 2):
    #         self.params['W' + str(i + 2)] = self.std * np.random.randn(self.hidden_sizes[i + 1],
    #                                                               self.hidden_sizes[i + 1])
    #         self.params['b' + str(i + 2)] = np.zeros(self.hidden_sizes[i + 1])

    #     self.params['W' + str(self.num_layer)] = self.std * np.random.randn(self.hidden_sizes[self.num_layer - 2],
    #                                                               self.num_classes)
    #     self.params['b' + str(self.num_layer)] = np.zeros(self.num_classes)
...
class MyOwnNetwork(ClassificationNet):
    def __init__(self, activation=Sigmoid, num_layer=2,
                 input_size=3 * 32 * 32, hidden_sizes=[100],
                 std=1e-3, num_classes=10, reg=0, **kwargs):
        """
        :param activation: choice of activation function. It should implement
            a forward() and a backward() method.
