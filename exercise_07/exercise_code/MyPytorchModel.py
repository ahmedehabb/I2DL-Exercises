...
class MyPytorchModel(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        for hidden_size in self.hparams["n_hiddens"]:
            layers.append(nn.Linear(in_features, hidden_size))
            layers.append(nn.BatchNorm1d(hidden_size))
            layers.append(nn.LeakyReLU())
            in_features = hidden_size  # Update input size for the next layer
            init.kaiming_uniform_(layers[-3].weight, mode='fan_in', nonlinearity='leaky_relu')
...
class CIFAR10DataModule(nn.Module):
    ...
    def prepare_data(self, stage=None, CIFAR_ROOT="../datasets/cifar10"):
        ...
        my_transform = transforms.Compose([
            transforms.RandomHorizontalFlip(),  # Example data augmentation: horizontal flip
            transforms.RandomRotation(10),      # Example data augmentation: random rotation (±10 degrees)
            transforms.RandomResizedCrop(32),   # Randomly resize and crop the image
            transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),  # Adjust color brightness, contrast, saturation, and hue
            transforms.RandomAffine(degrees=10, translate=(0.1, 0.1), scale=(0.9, 1.1), shear=10),  # Apply random affine transformations
