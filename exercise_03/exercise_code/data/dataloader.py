...
class DataLoader:
    ...
    def __iter__(self):
        ...
        for index in index_iterator:  # iterate over indices using the iterator
            ...
            if len(batch) == self.batch_size:
                yield self.batch_to_numpy(self.combine_batch_dicts(batch))  # use yield keyword to define a iterable generator
                batch = []
        
        # Check if there are remaining elements in the batch and drop_last = false
    ...
    def __len__(self):
        ...
        if self.drop_last == False and len(self.dataset) / self.batch_size > length:
            length += 1
